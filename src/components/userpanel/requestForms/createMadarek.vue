...
const props = defineProps(["bankName"])
...
const submit_madarek = async () => {
  if (cookies.get("gest_detail") != null) {
    submitState.value = true;
    let requestCookie = cookies.get("gest_detail");

    request.request_amount = requestCookie.requestedAmount
    request.return_period = requestCookie.returnPeriod
    request.payment_step = requestCookie.paymentStep
    request.monthly_payment = requestCookie.monthlyPayment
    request.total_Interest = requestCookie.totalInterest
    request.total_payment = requestCookie.totalPayment

    
    const send_madarek = await store.dispatch("send_madarek_files", {
    ...
    $toast.error(
      "لطفا قبل از ارسال درخواست ، از طریق فرم موجود در صفحه اصلی مبلغ قسط را مشخص کنید",
      {
        position: "top-left",
      }
    );
...
const clearFiles = () => {
  ...
  zamen_cart_melli.value.value = ""
  zamen_fish_hogugi.value.value = ""
  zamen_dovom_cartmelli.value.value = ""
  zamen_aval_madrakshoghli.value.value = ""
  zamen_aval_moadelsemahe.value.value = ""
  zamen_dovom_madrakshoghli.value.value = ""
  zamen_dovom_moadelsemahe.value.value = ""
